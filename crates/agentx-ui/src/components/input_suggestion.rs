//! A suggestion-enabled text input: an [`Input`] with an anchored popover that
//! offers items as the user types, driven entirely by props + callbacks.
//!
//! The parent owns the long-lived [`InputSuggestionState`] (which wraps the
//! shared [`InputState`]); [`InputSuggestion`] itself is a [`RenderOnce`] element
//! rebuilt each frame. This powers the composer's `@`-file and `/`-command
//! pickers. Self-contained — depends only on `gpui` + `gpui-component`.
